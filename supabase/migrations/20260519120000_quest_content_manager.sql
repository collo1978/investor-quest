-- Quest Content Manager — CMS cards for quest templates (Supabase-backed)
...
create table if not exists public.quest_content_cards (
  ...
  -- Empty array = available to all partners
...
create index if not exists quest_content_cards_active_idx
  on public.quest_content_cards (is_active) where is_active = true;
