...
export async function POST(request: Request) {
  ...
  if (!ticker || !questSlug) {
    return NextResponse.json(
      { error: "ticker and questSlug are required." },
      { status: 400 }
    );
  }
  ...
  try {
    const result = await runQuestBatchEvaluation({
      pillarId,
      ticker,
      ...
      useStoredAnswers: Boolean(body.useStoredAnswers),
      saveEvaluations:
        !body.useStoredAnswers &&
        body.saveEvaluations !== false &&
        Boolean(templateId)
    });
